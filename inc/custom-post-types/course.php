...
if ( ! class_exists( 'LP_Course_Post_Type' ) ) {
	...
	final class LP_Course_Post_Type extends LP_Abstract_Post_Type_Core {
		...
		public function add_script_data() {
			...
			if ( empty( $post ) || ( get_post_type() !== $this->_post_type ) || ! in_array(
				$pagenow,
				array(
					'post.php',
					'post-new.php',
		...
		public static function assessment_meta_box() {
			...
			$course_result_option_desc = array(
				...
				'evaluate_mark'       => __( 'Evaluate by achieved points of mark passed per total marks of all question.', 'learnpress' ),
			);
		...
		public static function payment_meta_box() {
			...
			if ( in_array(
				$role,
				apply_filters(
					'learn-press/user-set-course-price-roles',
					array(
						'admin',
						'instructor',
					)
